...
const SHUFFLE_MOVES_RANGE = [0, 80]; // TODO: add levels, 0, 10 === easy
// direction of cards
...
class GameState {
  ...
  // loop to check if board is solved
  isSolved() {
  ...
  undo() {
    ...
    this.board = this.stack.pop(); // pop method: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/pop
    this.moves -= 1; // subtraction assignment (-=) operator: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Subtraction_assignment
  }
  ...
  moveInDirection(dir) {
    ...
    const posToMove = dir === 'up' ? [epos[0] + 1, epos[1]] // ternary operator: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Conditional_Operator 
    ...
    const ARR_NUM_TILES = Array.from(Array(NUM_TILES).keys()).map(x => x + 1) // array.from static method: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/from
...
const Tile = ({ index, pos, onClick }) => {
  const top = pos[0] * 132;
  const left = pos[1] * 132;
  const bgLeft = (index % 3) * 132;
  const bgTop = Math.floor(index / 3) * 132;

  return <div
    className='absolute bg-transparent bg-tile bg-[length:400px_400px] transition-all duration-300 h-[130px] tile w-[130px]'
    onClick={onClick}
  ...
  return (
    <div className="bg-gradient-radial from-white via-white to-dark-yellow flex flex-col h-screen items-center place-content-center">
      ...
      <div className='cursor-move h-[400px] relative w-[400px] scale-75 md:scale-90 lg:scale-100 transition-all'>
        {
          board.slice(0, -1).map((pos, index) => ( // The slice() method: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/slice
            <Tile key={index} index={index} pos={pos} onClick={move(index)} />
          ))
        }
        {solved &&
          <div className='absolute bg-confetti flex h-[100%] justify-center opacity-70 w-[100%] z-10'>
